...
export default function GroupsPage() {
  ...
  const { users, unread, groupMessages, resetUnread, addToast } = useChatContext();
  ...
  return (
    <div className="chat-container">
      ...
      <div className="chat-main">
        <div className="messages" style={{ flex: 1 }}>
          {groupMessages.map(msg => (
            <div key={msg.id}
              className={`msg ${msg.sender_id === currentUserRef.current?.id ? 'mine' : 'other'}`}>
      ...
      <div className="toast-container">
        {/* global toasts are handled by context */}
      </div>
    </div>
  );
}
...
module.exports = function (io, socket) {
  ...
  if (!tokenPayload) { socket.disconnect(true); return; }

  // Temporary — will be replaced with DB data on JOIN
  socket.user = { id: Number(tokenPayload.id) };
  ...
  socket.on(EVENTS.JOIN, async () => {
    ...
    // Load the full user from DB — never trust the JWT for username.
    // The JWT may only carry { id, email } depending on how auth.service signs it.
    const dbUser = await usersRepo.findById(tokenPayload.id);
    if (!dbUser) { socket.disconnect(true); return; }

    socket.user = {
      id:       Number(dbUser.id),
      username: dbUser.username,   // guaranteed from DB
      email:    dbUser.email,
    };

    await presenceService.setOnline(socket.user.id, socket.id);

    // Broadcast enter — username always from DB, never from JWT
    socket.broadcast.emit(EVENTS.USER_ONLINE, {
      id:       socket.user.id,
      username: socket.user.username,
    });

    // Full user list (online flag included)
  ...
  socket.on('disconnect', async () => {
    setTimeout(async () => {
      ...
      // socket.user.username is set in JOIN; if JOIN never ran, look up DB
      ...
      io.emit(EVENTS.USER_OFFLINE, { id: Number(userId), username });
